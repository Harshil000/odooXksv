...
INSERT INTO vendors (user_id, company_name, gst_number, contact_person, phone, address, status)
...
export const SELECT_ALL_USERS_QUERY = `
SELECT u.id, u.full_name, u.email, u.role, u.is_active, u.created_at, u.updated_at,
       v.id AS vendor_id, v.company_name, v.gst_number, v.contact_person, v.phone, v.address, v.rating, v.status AS vendor_status
FROM users u
LEFT JOIN vendors v ON v.user_id = u.id
...
export const UPDATE_VENDOR_STATUS_QUERY = `
UPDATE vendors
SET status = $2
WHERE user_id = $1
RETURNING id, user_id, company_name, status;
`;